...
Скрипты с обучением моделей доступен в папке [train_models](train_models). Также в этой папке находится скрипт [norms.py](train_models/norms.py) - скрипт с нормами каждого параметра, к которому модели обращаются во время обучения.
...
Скрипт для предобработки вхолищих данных, получения предсказания и формирования заключения доступен по [ссылке](result_preparing.py)
