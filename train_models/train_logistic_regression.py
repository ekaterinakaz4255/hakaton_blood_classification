...
import pickle
import json
from norms import *  # Импортируем функции для норм
...
# 4. Обучение модели Logistic Regression с прогрессом
print("Обучение модели Logistic Regression...")
model = LogisticRegression(max_iter=1000)
with tqdm(total=100, desc="Обучение модели", unit="step") as pbar:
    model.fit(X_train, y_train)
    pbar.update(100)  # Обновляем прогресс на 100% (fit выполняется за один шаг)
...
if out_of_norm_results:
    ...
    with open("data/out_of_norm_results.json", "w") as f:
        import json
        json.dump(out_of_norm_results, f, indent=4)
    print("Отклонения сохранены в data/out_of_norm_results_logistic_regression.json")

# 8. Сохранение предсказаний
X_test_cleaned = X_test.copy()
X_test_cleaned["Predicted ICD-10"] = y_pred
X_test_cleaned.to_csv("data/test_with_predictions.csv", index=False)

print("Обучение модели Logistic Regression завершено!")