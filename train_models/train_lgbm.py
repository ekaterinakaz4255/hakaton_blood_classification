...
model_output_path = "models/lgbm.pkl"
        ...
        if param in row:
            try:
                ...
                if not (lower <= value <= upper):
                    out_of_norm[param] = {"value": value, "norm": norm}
...
# 4. Обучение модели LGBMClassifier с прогрессом
print("Обучение модели LGBMClassifier...")
model = LGBMClassifier(random_state=42, n_estimators=100, class_weight="balanced", learning_rate=0.1)
...
if out_of_norm_results:
    ...
    with open("data/out_of_norm_results_lgbm.json", "w") as f:
        json.dump(out_of_norm_results, f, indent=4)
    print("Отклонения сохранены в data/out_of_norm_results_lgbm.json")

# 8. Сохранение предсказаний
X_test_cleaned = X_test.copy()
X_test_cleaned["Predicted ICD-10"] = y_pred
X_test_cleaned.to_csv("data/test_with_predictions_lgbm.csv", index=False)

print("Обучение модели LGBMClassifier завершено!")
