...
model_output_path = "models/xgboost.pkl"
...
# 4. Обучение модели XGBoostClassifier с прогрессом
print("Обучение модели XGBoostClassifier...")
...
if out_of_norm_results:
    ...
    with open("data/out_of_norm_results_xgboost.json", "w") as f:
        json.dump(out_of_norm_results, f, indent=4)
    print("Отклонения сохранены в data/out_of_norm_results_xgboost.json")

# 8. Сохранение предсказаний
X_test_cleaned = X_test.copy()
X_test_cleaned["Predicted ICD-10"] = y_pred
X_test_cleaned.to_csv("data/test_with_predictions_xgboost.csv", index=False)

print("Обучение модели XGBoostClassifier завершено!")
